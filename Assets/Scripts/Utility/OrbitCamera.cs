...
public class OrbitCamera : MonoBehaviour
{
	...
	[SerializeField]
    private float xRotateConstraint = -6.5f;

	[SerializeField]
    private float zDistanceConstraint = 1f;
...
//if (Serializer.IsLoading) return;
	...
	void Rotate (float x, float y)
	{
		...
		Vector3 position = rotation * _distanceVector + _target.position + new Vector3 (0.0f, .1f, 0.0f);
		;
	...
	void Zoom ()
	{
		if ((Input.mousePosition.x < 200) || (Input.mousePosition.x > 1600)) return;
